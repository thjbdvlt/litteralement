...
create table onto.tag (like onto.classe including all);

-- les classes qui contiennent les instances (entités, propriétés, relations, instance_a_tag, propriete_value) de la partie public.
...
-- les tables qui nécessitent des valeurs ont, pour chaque datatype, une table correspondante.
...
create table public.atag (
    entite int references public.entite(id) not null,
    tag int references onto.tag(id) not null,
    unique (entite, tag)
);

-- la table qui fait la liaison entre les deux parties de la base de données (EAV et relationnel standard) est la table texte.

create table nlp.texte(
    -- la table 'texte' est différente de la table 'prop_text' car elle contient des objets avec un identifiant, auquel peut faire référence les tables du schéma 'nlp'.
    id int primary key 
        generated by default as identity not null,
    val text
) inherits(propriete);

-- la partie relationnelle standard de la base de données, pour l'analyse linguistique qui se fait à l'aide de catégories, d'outils conceptuels indépendant des objets matériels et sociaux qui contiennent les textes.

-- première partie décrit les mots possibles et les types de segment, elle est en quelque sorte idéale et abstraite.

create table nlp.nature (like onto.tag including all);
create table nlp.fonction (like onto.tag including all);
...
create table nlp.lexeme (
    ...
    morph_feats smallint references nlp.morph(id) not null,
    morph_hunspell smallint references nlp.morph(id) not null,
    graphie text not null
);

-- seconde partie décrit les mots et segments réels, utilisés dans des contextes (des textes).
