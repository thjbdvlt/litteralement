...
def add_user_defined_columns(conn, table, userattrs):
    """Ajoute les colonnes user-defined (et tables associés).
    ...
    for i in userattrs:
        ...
        if i['is_literal'] is True:
            ...
            s = SQL("create table if not exists {table} (id integer primary key generated by default as identity, {col} {datatype})")
            s = s.format(table=name, col=value_column, datatype=datatype)
            conn.execute(s)
            s = SQL("alter table {} add column if not exists {} integer references {} (id)")
...
def _insert_lexemes(conn, **kwargs):
    ...
    if "lex_user_attrs" in kwargs:
        userattrs = kwargs["lex_user_attrs"]
    ...
    stmt = SQL("insert into lexeme ({columns})").format(columns=insert_columns)
